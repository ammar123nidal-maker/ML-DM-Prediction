...
st.write("أدخل بيانات المريض لمعرفة **الاحتمال التقديري** للإصابة بمرض السكري:")
...
gender_other = 0  # إذا النموذج لا يحتوي فئة أخرى، خليه صفر دائمًا
...
if st.button("🔍 التنبؤ"):
    ...
    if prediction[0] == 1:
        st.warning("⚠️ تشير النتيجة إلى **احتمال مرتفع للإصابة بمرض السكري**.\nيُفضل استشارة الطبيب لإجراء فحوصات دقيقة.")
    else:
        st.info("✅ تشير النتيجة إلى **احتمال منخفض للإصابة بمرض السكري**.\nمع ذلك، يُنصح بالحفاظ على نمط حياة صحي والمتابعة الدورية.")
